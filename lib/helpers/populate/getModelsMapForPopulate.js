...
module.exports = function getModelsMapForPopulate(model, docs, options) {
  ...
  for (i = 0; i < len; i++) {
    ...
    let foreignFieldDefined = null;
    if (allSchemaTypes.length) {
      const schemaTypeDefined = allSchemaTypes.filter(v => v.path === options.path);
      if (schemaTypeDefined[0]) {
        foreignFieldDefined = schemaTypeDefined[0].options && schemaTypeDefined[0].options.foreignField;
        if (!foreignFieldDefined && schemaTypeDefined[0].options.type && schemaTypeDefined[0].options.type[0] && schemaTypeDefined[0].options.type[0].foreignField) {
          foreignFieldDefined = schemaTypeDefined[0].options.type[0].foreignField;
        }
      }
    }

    let foreignField = virtual && virtual.options ?
      virtual.options.foreignField : (foreignFieldDefined ? foreignFieldDefined : '_id');
